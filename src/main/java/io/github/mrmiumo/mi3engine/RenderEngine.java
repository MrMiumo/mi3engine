...
public class RenderEngine {
    ...
    private static final int[][] FACE_VERTEX_IDS = {
        {1,5,7,3}, // +X
        {0,2,6,4}, // -X
        {2,3,7,6}, // +Y
        {0,4,5,1}, // -Y
        ...
        for (Cube cube : cubes) {
            ...
            for (var face : Face.values()) {
                ...
                for (int k = 0 ; k < 4 ; k++) {
                    ...
                    Vec view = world.rotate(config.camRotation()).sub(config.camTranslation());
     ...
     */
    private Vec[] localVertices(Cube cube) {
        Vec s = cube.size();
     ...
     */
    private Function<Vec, Vec> modelToWorld(Cube cube) {
